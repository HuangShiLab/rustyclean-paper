#!/usr/bin/env python3
"""Compute accuracy for 100M benchmark: RustyClean auto/sylph, KneadData, Hostile."""
import csv
import gzip
import sys
...
def find_clean_files(outdir: Path, tool: str) -> list[Path]:
    """Find cleaned FASTQ files for various tools."""
    candidates = []
    if tool == "kneaddata":
        patterns = ["*clean*.fastq*", "*paired*.fastq*", "*unmatched*.fastq*"]
    elif tool == "hostile":
    ...
    if not filtered:
        return []
    # For RustyClean, prefer _clean_R1 or _clean
...
def process_item(args):
    ...
    if tool.startswith("rustyclean_"):
        rc_mode = tool.split("_", 1)[1]  # auto or sylph
        outdir = results_base / f"results_{rc_mode}" / dataset / f"rep_{rep}"
    else:
        outdir = results_base / "results" / dataset / f"rep_{rep}"
...
def main():
    ...
    tools = ["rustyclean_auto", "rustyclean_sylph", "kneaddata", "hostile"]
    reps = [1, 2, 3]

    args = [(data_dir, tool, ds, rep, results_base) for ds in datasets for tool in tools for rep in reps]
    print(f"Processing {len(args)} tool/dataset/rep combinations using up to {cpu_count()} cores")
