#!/usr/bin/env python3
"""
Accuracy analysis for cross-species benchmark.
"""
...
def process_tool(data_dir, results_dir, tool_name, rows):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        for dataset in datasets:
            ...
            species = dataset.split('_')[0]
            dataset_dir = tool_dir / dataset

            if tool_name == 'rustyclean':
                ...
                for rep_dir in rep_dirs:
                    ...
                    if not fq:
                        continue
                    _process(fq, tmpdir, dataset, species, rep, tool_name, gt_norm, rows)
            else:
                fq = find_clean_fastq(dataset_dir)
                if not fq:
                    continue
                _process(fq, tmpdir, dataset, species, '1', tool_name, gt_norm, rows)


def _process(fq, tmpdir, dataset, species, rep, tool_name, gt_norm, rows):
    ...
    print(f"  {dataset} rep{rep}: f1={metrics['f1']:.4f}", flush=True)
...
def main():
    ...
    out_csv = output_dir / 'accuracy_cross_species.csv'
