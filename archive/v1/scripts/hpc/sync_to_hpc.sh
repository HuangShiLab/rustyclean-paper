...
#
# Usage:
#   bash scripts/hpc/sync_to_hpc.sh [user@hpc_host]
#
# Example:
#   bash scripts/hpc/sync_to_hpc.sh shihuang@hpc.example.com
...
HPC_HOST="${1:-}"
if [ -z "$HPC_HOST" ]; then
    echo "Usage: bash scripts/hpc/sync_to_hpc.sh [user@hpc_host]"
    echo ""
...
echo "  bash scripts/hpc/setup_hpc_env.sh"
echo "  cd genomes && bash download_genomes.sh"
echo "  cd $HPC_PROJECT_DIR"
echo "  bash scripts/hpc/submit_all.sh"
