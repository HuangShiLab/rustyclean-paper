...
DATA_DIR="$PROJECT_DIR/scripts/minimal/data"
RESULTS_DIR="$PROJECT_DIR/scripts/minimal/results"
THREADS=8
...
if [ -f "$PROJECT_DIR/minimal_env/env.sh" ]; then
    ...
    echo "ERROR: 环境未设置。请先运行: bash scripts/minimal/setup_minimal_env.sh"
    exit 1
fi
...
echo ""
echo "如果结果满意，运行: bash scripts/minimal/upgrade_to_standard.sh"
