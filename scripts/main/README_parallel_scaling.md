...
先跑的臂要冷读 lustre 上的输入,读完之后面的四个臂就是热缓存了——**臂的顺序本身值几分钟的
I/O 等待**,而这正落在要比较的数字里面。所以正式计时前会先把输入和五个臂各自的索引各读一遍
(不计时),让五个臂从同一缓存状态出发。

这意味着测的是**热缓存**性能。这是可复现、且工具之间公平的选择;但它不等于用户第一次对着
冷存储跑的体验,报告里要写明。`PARALLEL_WARM_CACHE=0` 可关掉。
