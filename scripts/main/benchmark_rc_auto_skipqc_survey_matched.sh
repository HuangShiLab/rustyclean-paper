#!/bin/bash
#SBATCH --job-name=rc_auto_skipqc_survey_matched
...
#SBATCH --array=0-3
#SBATCH --output=/home/%u/rc_auto_skipqc_survey_matched_%A_%a.out
#SBATCH --error=/home/%u/rc_auto_skipqc_survey_matched_%A_%a.err
...
RESULTS_DIR="${RUNS_DIR:-/lustre1/g/aos_shihuang/rustyclean-paper/runs}/rustyclean_auto_skipqc_survey_matched"
...
for REP in 1 2 3; do
    OUT="${RESULTS_DIR}/rustyclean_auto_skipqc_survey/${DATASET}/rep_${REP}"
    rm -rf "${OUT}"
    mkdir -p "${OUT}"

    logfile="${LOGS_DIR}/rustyclean_auto_skipqc_${DATASET}_rep${REP}.log"
    timefile="${LOGS_DIR}/rustyclean_auto_skipqc_${DATASET}_rep${REP}.time"

    echo "  [rep ${REP}] Running RustyClean --auto --skip-qc on ${DATASET}..."
    ...
    /usr/bin/time -v -o "${timefile}" \
        ...
        echo "rustyclean_auto_skipqc_survey,${DATASET},${REP},FAILED,,$(date -Iseconds)" >> "${METRICS_DIR}/performance.csv"
        continue
    }
    ...
    ts=$(date -Iseconds)
    echo "rustyclean_auto_skipqc_survey,${DATASET},${REP},${runtime_sec},${max_mem},${ts}" >> "${METRICS_DIR}/performance.csv"
