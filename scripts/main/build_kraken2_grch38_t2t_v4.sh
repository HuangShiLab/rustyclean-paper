#!/bin/bash
#SBATCH --job-name=k2_grch38_t2t_v4
...
#SBATCH --output=/home/shihuang/k2_grch38_t2t_v4_%j.out
#SBATCH --error=/home/shihuang/k2_grch38_t2t_v4_%j.err
...
K2_DB="${DB_DIR}/kraken2/grch38_t2t_v4"
...
# Decompress input FASTA (kraken2-build --add-to-library requires plain text)
echo "Decompressing GRCh38..."
zcat "${GRCH38}" > "${K2_DB}/library/grch38.fna"
echo "Decompressing T2T..."
zcat "${T2T}" > "${K2_DB}/library/t2t.fna"

# Add GRCh38 to library
echo "[1/3] Adding GRCh38 to library..."
kraken2-build --add-to-library "${K2_DB}/library/grch38.fna" --db "${K2_DB}" --threads 8 2>&1 | tee "${K2_DB}/add_library.log"

# Add T2T to library
echo "[2/3] Adding T2T-CHM13 to library..."
kraken2-build --add-to-library "${K2_DB}/library/t2t.fna" --db "${K2_DB}" --threads 8 2>&1 | tee -a "${K2_DB}/add_library.log"
