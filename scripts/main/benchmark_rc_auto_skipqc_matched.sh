#!/bin/bash
#SBATCH --job-name=rc_auto_skipqc_matched
...
for REP in 1 2 3; do
    OUT="${RESULTS_DIR}/rustyclean_auto_skipqc/${DATASET}/rep_${REP}"
    rm -rf "${OUT}"
    mkdir -p "${OUT}"
    ...
    /usr/bin/time -v -o "${timefile}" \
        ...
        echo "rustyclean_auto_skipqc,${DATASET},${REP},FAILED,,$(date -Iseconds)" >> "${METRICS_DIR}/performance.csv"
        continue
    }
    ...
    ts=$(date -Iseconds)
    echo "rustyclean_auto_skipqc,${DATASET},${REP},${runtime_sec},${max_mem},${ts}" >> "${METRICS_DIR}/performance.csv"
