#!/bin/bash
#SBATCH --job-name=bt2_grch38_t2t
...
#SBATCH --output=/home/shihuang/bt2_grch38_t2t_%j.out
#SBATCH --error=/home/shihuang/bt2_grch38_t2t_%j.err
...
echo "Building bowtie2 index: ${OUT_PREFIX}"

# Decompress to temp for bowtie2-build
TMP_FA="${DB_DIR}/fasta/grch38_t2t_merged_genomic.fna"
zcat "${FASTA}" > "${TMP_FA}"

bowtie2-build --threads 8 "${TMP_FA}" "${OUT_PREFIX}" 2>&1 | tee "${DB_DIR}/bowtie2/build.log"

# Remove uncompressed FASTA to save space
rm -f "${TMP_FA}"
