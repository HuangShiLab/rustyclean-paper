...
3. **Conda 环境** 已安装所有依赖（参考 `scripts/setup_env.sh` 或 `minimal/setup_minimal_env.sh`）。
4. **数据库** 已下载到共享目录：
...
```bash
# 1. 生成数据
GEN_JOB=$(sbatch --parsable hpc/generate_data_slurm.sh)

# 2. 运行 benchmark（依赖数据生成）
BENCH_JOB=$(sbatch --parsable --dependency=afterok:$GEN_JOB hpc/run_benchmark_slurm.sh)

# 3. 下游分析
DOWN_JOB=$(sbatch --parsable --dependency=afterok:$BENCH_JOB hpc/downstream_slurm.sh)

# 4. 分析与可视化
sbatch --dependency=afterok:$DOWN_JOB hpc/analyze_slurm.sh
```
...
sbatch --array=<task_id> hpc/generate_data_slurm.sh
```
...
```bash
bash hpc/run_single_benchmark.sh <dataset_name> <rep>
```
